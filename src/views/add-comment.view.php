...
<main class="flex h-screen bg-black text-gray-200 items-center justify-center p-6">
    <div class="w-full max-w-md bg-gray-900 rounded-2xl shadow-lg p-8 border border-gray-800">
        ...
        <form action="/insert-comment" method="POST" class="flex flex-col gap-4">
            <input type="hidden" name="book_id" value="<?= $book['id']?>">
            <input type="hidden" name="user_id" value="<?= $_SESSION['user']['id']?>">
            <textarea name="description" id="description" rows="5" placeholder="Escribe tu comentario aquí..."
                      class="px-4 py-2 rounded border border-gray-700 bg-gray-800 text-gray-200 focus:outline-none focus:border-indigo-400"></textarea>
