...
<main class="min-h-screen bg-gradient-to-b from-gray-950 to-gray-900 text-gray-200 flex flex-col items-center p-8">
    ...
    <section class="w-full max-w-3xl bg-gray-900/70 border border-gray-800 rounded-2xl shadow-lg p-8 backdrop-blur-sm">
        <div class="space-y-4 text-lg">
            <div class="flex justify-between border-b border-gray-800 pb-2">
                <span class="text-indigo-400 font-semibold">ID:</span>
                <span><?= htmlspecialchars($book["id"]) ?></span>
            </div>

            <div class="flex justify-between border-b border-gray-800 pb-2">
                <span class="text-indigo-400 font-semibold">Título:</span>
                <span><?= htmlspecialchars($book["title"]) ?></span>
            </div>

            <div class="flex justify-between border-b border-gray-800 pb-2">
                <span class="text-indigo-400 font-semibold">Autor:</span>
                <span><?= htmlspecialchars($book["author"]) ?></span>
            </div>

            <div class="flex justify-between border-b border-gray-800 pb-2">
                <span class="text-indigo-400 font-semibold">Publicado:</span>
                <span><?= htmlspecialchars($book["publish_date"]) ?></span>
            </div>
        </div>
    </section>
    ...
    <section class="w-full max-w-3xl mt-10 bg-gray-900/70 border border-gray-800 rounded-2xl shadow-lg p-8 backdrop-blur-sm">
        ...
        <?php if (empty($comments)): ?>
            <p class="text-gray-400 italic">Aún no hay comentarios para este libro...</p>
        <?php endif; ?>
    </section>
    ...
    <div>
        <label for="description" class="block text-gray-400 mb-1">Comentario:</label>
        <textarea id="description" name="description" rows="3" required
            class="w-full p-3 bg-gray-800 text-gray-200 border border-gray-700 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none"></textarea>
    </div>

    <div>
        <label for="rating" class="block text-gray-400 mb-1">Puntuación:</label>
        <select id="rating" name="rating" required
            class="w-full p-3 bg-gray-800 text-gray-200 border border-gray-700 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none">
    ...
    <button type="submit"
        class="w-full py-2 bg-indigo-600 hover:bg-indigo-700 rounded-xl font-semibold text-white transition">
        Enviar comentario
    </button>
</form>
